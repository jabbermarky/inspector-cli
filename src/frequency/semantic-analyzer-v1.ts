...
function isCMSHeader(headerName: string): boolean {
    ...
    return cmsPatterns.some(
        pattern => headerName.startsWith(pattern) || headerName.includes(pattern)
    );
}
...
function isEcommerceHeader(headerName: string): boolean {
    ...
    return ecommercePatterns.some(
        pattern => headerName.includes(pattern) || headerName.startsWith(pattern)
    );
}
...
function detectAnalyticsVendor(headerName: string): string | undefined {
    if (headerName.includes('google') || headerName.includes('ga-') || headerName.includes('gtm-'))
        return 'Google';
...
function detectCMSVendor(headerName: string): string | undefined {
    if (
        headerName.includes('wp-') ||
        headerName.includes('wordpress') ||
        headerName.includes('pingback')
    )
        return 'WordPress';
...
function detectEcommerceVendor(headerName: string): string | undefined {
    if (
        headerName.includes('shopify') ||
        headerName.includes('shopid') ||
        headerName.includes('shardid')
    )
        return 'Shopify';
...
function detectNamingConvention(
    headerName: string
): 'kebab-case' | 'camelCase' | 'underscore_case' | 'UPPER_CASE' | 'mixed' | 'non-standard' {
    ...
    if (
        (hasHyphens && hasUnderscores) ||
        (hasHyphens && hasMixedCase) ||
        (hasUnderscores && hasMixedCase)
    ) {
        return 'mixed';
    }
...
function determinePatternType(
    headerName: string,
    category: HeaderCategory
): 'standard' | 'vendor-specific' | 'platform-specific' | 'custom' {
...
export function generateSemanticInsights(
    ...
    for (const analysis of analyses.values()) {
        ...
        if (analysis.category.vendor) {
            vendorDistribution[analysis.category.vendor] =
                (vendorDistribution[analysis.category.vendor] || 0) + 1;
        }

        // Count naming conventions
        namingConventions[analysis.namingConvention] =
            (namingConventions[analysis.namingConvention] || 0) + 1;
    ...
    const topCategories = Object.entries(categoryDistribution)
        .map(([category, count]) => ({
            category: category as HeaderPrimaryCategory,
            count,
