...
export class CooccurrenceAnalyzerV2 implements FrequencyAnalyzer<CooccurrenceSpecificData> {
  ...
  async analyze(
    ...
    const cooccurrences = this.calculateCooccurrenceStatistics(headerMatrix, data.totalSites, options);
    ...
    logger.info('Detected mutual exclusivity groups', { groups: mutualExclusivityGroups.length });

    // Step 6: Extract strong correlations
    const strongCorrelations = this.extractStrongCorrelations(cooccurrences);

    // Step 7: Generate insights
    const insights = this.generateInsights(cooccurrences, technologySignatures, platformCombinations, mutualExclusivityGroups);
  ...
  private calculateCooccurrenceStatistics(
    ...
    for (let i = 0; i < headers.length; i++) {
      for (let j = i + 1; j < headers.length; j++) {
        ...
        const mutualInformation = jointProbability > 0 && expectedJoint > 0
    ...
    const sortedCooccurrences = new Map(
      [...cooccurrences.entries()].sort(([,a], [,b]) => b.mutualInformation - a.mutualInformation)
    );
    ...
    for (const [cms, sites] of cmsSites.entries()) {
      ...
      for (const combination of headerCombinations) {
        if (combination.headers.length >= 2) {
          const exclusivity = this.calculatePlatformExclusivity(combination.headers, cms, cmsSites);
   ...
   */
  private groupSitesByCMS(data: PreprocessedData): Map<string, Array<{url: string, headers: Set<string>}>> {
  ...
  private findCommonHeaderCombinations(
    ...
    for (let i = 0; i < headerArray.length; i++) {
      for (let j = i + 1; j < headerArray.length; j++) {
        ...
        const sitesWithBoth = sites.filter(site => 
          site.headers.has(header1) && site.headers.has(header2)
        );
  ...
  private detectMutualExclusivity(
    ...
    const lowCooccurrencePairs = Array.from(cooccurrences.values()).filter(c => 
    ...
    for (const [header, exclusives] of exclusivityMap.entries()) {
      ...
      if (group.length >= 3) {
        const avgExclusivity = Array.from(group).reduce((sum, h) => {
  ...
  private generateInsights(
    ...
    const topSignature = Array.from(technologySignatures.values())
    ...
    const topCombination = Array.from(platformCombinations.values())
      .sort((a, b) => (b.frequency * b.exclusivity) - (a.frequency * a.exclusivity))[0];
    if (topCombination) {
      insights.push(
    ...
    if (mutualExclusivityGroups.length > 0) {
      const largest = mutualExclusivityGroups.reduce((a, b) => a.headers.length > b.headers.length ? a : b);
      insights.push(
  ...
  private createStatisticalSummary(
    ...
    const avgMI = cooccurrenceArray.reduce((sum, c) => sum + c.mutualInformation, 0) / cooccurrenceArray.length;
    ...
    for (const [pairKey, cooccurrence] of cooccurrences) {
      if (cooccurrence.cooccurrenceCount >= options.minOccurrences) {
        ...
        patterns.set(`cooccurrence:${pairKey}`, {
          ...
          examples: options.includeExamples ? new Set([`${cooccurrence.header1} + ${cooccurrence.header2}`]) : undefined,
          metadata: {
            type: 'cooccurrence',
