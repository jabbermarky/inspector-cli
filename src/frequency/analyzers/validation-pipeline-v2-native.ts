...
export interface ValidationStage {
  name: string;
  description: string;
  validate(data: PreprocessedData, options: AnalysisOptions, context: ValidationContext): Promise<ValidationStageResult>;
}
...
export class ValidationPipelineV2Native implements FrequencyAnalyzer<ValidationSpecificData> {
   ...
   */
  private applyStageEnhancements(data: PreprocessedData, enhancements: Partial<PreprocessedData>): void {
  ...
  private calculateFinalQualityMetrics(context: ValidationContext): QualityMetrics {
    ...
    if (stageScores.length > 0) {
      metrics.dataCompleteness = this.calculateComponentScore(context.stageResults, 'data_completeness');
      metrics.statisticalReliability = this.calculateComponentScore(context.stageResults, 'statistical_reliability');
      metrics.patternConsistency = this.calculateComponentScore(context.stageResults, 'pattern_consistency');
      metrics.correlationStrength = this.calculateComponentScore(context.stageResults, 'correlation_strength');
      metrics.sampleAdequacy = this.calculateComponentScore(context.stageResults, 'sample_adequacy');
      metrics.distributionHealth = this.calculateComponentScore(context.stageResults, 'distribution_health');
    }
   ...
   */
  private calculateComponentScore(stageResults: ValidationStageResult[], component: string): number {
  ...
  private performChiSquareTests(context: ValidationContext): ChiSquareResult[] {
    ...
    if (patternsByCms.size >= 2 && context.validatedPatterns.size >= 2) {
      ...
      if (topPatterns.length === 2 && topCms.length === 2) {
        ...
        const contingencyTable = [[0, 0], [0, 0]];
        ...
        if (total > 0 && contingencyTable.every(row => row.some(val => val > 0))) {
          const chiSquareResult = StatisticalUtils.ChiSquare.chiSquareTest(contingencyTable);
          
          // Only add result if calculations are valid
          if (Number.isFinite(chiSquareResult.statistic) && Number.isFinite(chiSquareResult.pValue)) {
            results.push({
  ...
  private performCorrelationTests(context: ValidationContext): CorrelationTestResult[] {
    ...
    for (let i = 0; i < Math.min(patterns.length, 5); i++) {
      for (let j = i + 1; j < Math.min(patterns.length, 5); j++) {
        ...
        const correlation = intersection.size / Math.sqrt(pattern1.sites.size * pattern2.sites.size);
        ...
        results.push({
          ...
          confidenceInterval: [Math.max(0, correlation - 0.1), Math.min(1, correlation + 0.1)]
        });
      }
    }
  ...
  private performPowerAnalysis(context: ValidationContext): PowerAnalysisResult {
    ...
    return StatisticalUtils.PowerAnalysis.calculateStatisticalPower(
      totalSites,
      minFrequency
    );
  }
  ...
  private compileRecommendations(context: ValidationContext): ValidationRecommendation[] {
    ...
    return recommendations.sort((a, b) => b.priority - a.priority || b.confidence - a.confidence);
  }

  /**
   * Create validation summary
   */
  private createValidationSummary(context: ValidationContext, metrics: QualityMetrics): ValidationSpecificData['validationSummary'] {
   ...
   */
  private generateImprovementSuggestions(context: ValidationContext, metrics: QualityMetrics): string[] {
   ...
   * Create V2 patterns for interface compatibility
   */
  private createValidationPatterns(context: ValidationContext, options: AnalysisOptions): Map<string, PatternData> {
    const patterns = new Map<string, PatternData>();
