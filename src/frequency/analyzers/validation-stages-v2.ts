...
export class DistributionValidationStage implements ValidationStage {
    ...
    async validate(
        ...
        const passed = errors.length === 0;
        const score = this.calculateDistributionScore(
...
export class CorrelationValidationStage implements ValidationStage {
    ...
    async validate(
        ...
        const suspiciousCorrelations = correlationAnalysis.filter(
            c => Math.abs(c.correlation) > 0.95
        );
    ...
    private analyzeCMSDistribution(data: PreprocessedData): {
        balance: number;
    ...
    private validateCMSCorrelations(
        patterns: Map<string, any>,
        data: PreprocessedData
    ): Array<{
        pattern: string;
    ...
    private calculateCorrelationScore(
        ...
        const correlationScore =
            correlationAnalysis.length > 0
                ? correlationAnalysis.reduce((sum, c) => sum + Math.abs(c.correlation), 0) /
...
export class SanityValidationStage implements ValidationStage {
            ...
            if (result.warnings) {
                for (const warning of result.warnings) {
                    warnings.push({
                        ...
                        affectedPatterns: warning.headerName
                            ? [warning.headerName]
                            : ['correlation_system'],
    ...
    private async executeAllSanityChecks(
        ...
        try {
            ...
            results.bayesianConsistency = this.bayesianConsistencyCheck(
                correlations,
                cmsDistribution
            );
    ...
    private correlationSumCheck(correlations: Map<string, any>): any {
        ...
        for (const [headerName, correlation] of correlations) {
            const cmsGivenHeaderSum = Object.values(correlation.cmsGivenHeader).reduce(
                (sum: number, data: any) => sum + data.probability,
                0
            );
    ...
    private supportCheck(correlations: Map<string, any>): any {
        ...
        for (const [headerName, correlation] of correlations) {
            for (const [cmsName, data] of Object.entries(correlation.cmsGivenHeader)) {
                ...
                if (
                    dataTyped.probability > highCorrelationThreshold &&
                    dataTyped.count < minSampleSizeForHighCorr
                ) {
                    warnings.push({
                        severity: 'medium',
    ...
    private bayesianConsistencyCheck(correlations: Map<string, any>, cmsDistribution: any): any {
        const warnings: any[] = [];
        const totalSites = Object.values(cmsDistribution).reduce(
            (sum: number, cms: any) => sum + cms.count,
            0
        );
        ...
        for (const [headerName, correlation] of correlations) {
            ...
            for (const [cmsName, cmsGivenHeaderData] of Object.entries(
                correlation.cmsGivenHeader
            )) {
                ...
                const relativeError =
                    Math.abs(leftSide - rightSide) / Math.max(leftSide, rightSide, 0.001);
    ...
    private probabilityConservationCheck(correlations: Map<string, any>): any {
        for (const [headerName, correlation] of correlations) {
            ...
            const cmsCountSum = Object.values(correlation.cmsGivenHeader).reduce(
    ...
    async validate(
            ...
            if (significanceTest.significant) {
                ...
                context.flaggedPatterns.set(key, {
                    ...
                    metadata: {
                        ...
                        testType: significanceTest.testType,
                    },
                });
            }
        }
        ...
        if (significanceRate < 0.5) {
            warnings.push({
                ...
                suggestedAction:
                    'Consider increasing sample size or adjusting significance criteria',
            });
        }
...
export class RecommendationValidationStage implements ValidationStage {
    ...
    async validate(
        ...
        const passed = errors.length === 0;
        const score =
            (qualityMetrics.averageConfidence + (consistencyCheck.consistent ? 1 : 0.5)) / 2;
    ...
    private validateRecommendationConsistency(recommendations: ValidationRecommendation[]): {
        ...
        if (sampleSizeRecs.length > 1) {
            ...
            const increase = sampleSizeRecs.some(r =>
                r.actionableSteps.some(s => s.includes('Increase'))
            );
            const decrease = sampleSizeRecs.some(r =>
                r.actionableSteps.some(s => s.includes('Decrease'))
            );
