...
export interface EmergingVendorPattern {
  ...
  characteristics: {
    ...
    semanticCategories: HeaderPrimaryCategory[];
  ...
  technologyStack?: {
    ...
    confidence: number;
    stackCategory: 'cms' | 'ecommerce' | 'cdn' | 'analytics' | 'framework' | 'hosting' | 'security';
  };
}
...
export interface SemanticAnomaly {
  headerName: string;
  expectedCategory: HeaderPrimaryCategory;
  actualCategory: HeaderPrimaryCategory;
  confidence: number;
  reason: string;
  severity: 'low' | 'medium' | 'high';
  sites: string[];
  frequency: number;
  anomalyType: 'category-mismatch' | 'vendor-mismatch' | 'naming-inconsistency' | 'semantic-drift';
  context?: {
...
export class PatternDiscoveryV2 implements FrequencyAnalyzer<PatternDiscoverySpecificData> {
  ...
  async analyze(
    ...
    const insights = this.generateInsights(discoveredPatterns, emergingVendors, patternEvolution, semanticAnomalies, patternNetworks);

    // Step 8: Calculate discovery metrics
    const discoveryMetrics = this.calculateDiscoveryMetrics(discoveredPatterns, emergingVendors, patternEvolution, semanticAnomalies, data);
   ...
   */
  private buildEnhancedHeaderFrequencyMap(data: PreprocessedData): Map<string, {
    frequency: number;
    siteCount: number;
    sites: string[];
    cmsCorrelation: Record<string, number>;
  ...
  private discoverNamingPatterns(
    ...
    const prefixPatterns = this.discoverPrefixPatterns(headers, headerFrequencyMap, data.totalSites);
    prefixPatterns.forEach(pattern => patterns.set(`prefix:${pattern.pattern}`, pattern));

    // Discover suffix patterns
    const suffixPatterns = this.discoverSuffixPatterns(headers, headerFrequencyMap, data.totalSites);
    suffixPatterns.forEach(pattern => patterns.set(`suffix:${pattern.pattern}`, pattern));

    // Discover contains patterns
    const containsPatterns = this.discoverContainsPatterns(headers, headerFrequencyMap, data.totalSites);
    containsPatterns.forEach(pattern => patterns.set(`contains:${pattern.pattern}`, pattern));

    // Discover regex patterns
    const regexPatterns = this.discoverRegexPatterns(headers, headerFrequencyMap, data.totalSites);
    ...
    const minFrequency = data.totalSites >= 100 ? 0.05 : 0.10;
    const minOccurrences = options.minOccurrences;

    const filteredPatterns = new Map<string, DiscoveredPattern>();
    for (const [key, pattern] of patterns) {
      const meetsThreshold = pattern.frequency >= minFrequency && pattern.siteCount >= minOccurrences;
      const isValidatedPattern = pattern.validationConfidence && pattern.validationConfidence > 0.7;
    ...
    return new Map([...filteredPatterns.entries()]
      .sort(([,a], [,b]) => {
  ...
  private discoverPrefixPatterns(
    ...
    for (const [prefix, matchingHeaders] of prefixMap.entries()) {
      if (matchingHeaders.length >= 2) {
        ...
        const validationConfidence = validatedHeaderCount > 0 
        ...
        patterns.push({
          ...
          statisticalSignificance: this.calculateStatisticalSignificance(frequency, allSites.size),
    ...
    for (const [suffix, matchingHeaders] of suffixMap.entries()) {
      if (matchingHeaders.length >= 2) {
        ...
        patterns.push({
          ...
          examples: matchingHeaders.slice(0, 5),
          confidence,
          validationConfidence: validatedCount > 0 ? validationConfidence / validatedCount : undefined,
          statisticalSignificance: this.calculateStatisticalSignificance(frequency, allSites.size),
          cmsCorrelation
        });
      }
  ...
  private discoverRegexPatterns(
    ...
    const regexTemplates = [
      { pattern: /^x-[a-z]+-[a-z]+$/, name: 'x-word-word', description: 'X-prefixed compound headers' },
      { pattern: /^[a-z]+-[a-z]+-[a-z]+$/, name: 'word-word-word', description: 'Triple compound headers' },
      { pattern: /^[a-z]+_[a-z]+$/, name: 'word_word', description: 'Underscore compound headers' },
      ...
      { pattern: /^[a-z]+-version$/, name: 'word-version', description: 'Version suffix headers' },
      ...
      { pattern: /^[a-z]+-status$/, name: 'word-status', description: 'Status suffix headers' }
    ];
  ...
  private inferVendorFromToken(token: string): string | undefined {
    ...
    const commonWords = ['content', 'cache', 'request', 'response', 'session', 'user', 'api', 'data', 'version', 'total', 'time', 'status', 'header', 'value', 'test'];
  ...
  private identifyEmergingVendors(
    ...
    for (const [vendorName, patterns] of vendorGroups.entries()) {
      ...
      if (patterns.length >= minPatterns) {
        ...
        // Extract semantic categories from examples
        const semanticCategories = patterns
          .flatMap(p => p.examples)
          .map(header => {
            try {
              const analysis = analyzeHeaderSemantics(header);
              return analysis.category.primary;
            } catch {
              return 'other' as HeaderPrimaryCategory;
            }
          });
        ...
        vendors.set(vendorName, {
          ...
          patterns,
          characteristics: {
            namingConvention: this.analyzeNamingConvention(patterns[0].examples[0] || ''),
  ...
  private inferHeaderStructure(patterns: DiscoveredPattern[]): string {
    ...
    const structures = patterns.flatMap(p => p.examples).map(header => {
  ...
  private calculateCategoryDistribution(categories: HeaderPrimaryCategory[]): Record<string, number> {
  ...
  private inferStackCategory(vendorName: string, categories: HeaderPrimaryCategory[]): 'cms' | 'ecommerce' | 'cdn' | 'analytics' | 'framework' | 'hosting' | 'security' {
    // Simple heuristics for stack categorization
    if (vendorName.includes('wp') || vendorName.includes('wordpress') || vendorName.includes('drupal')) return 'cms';
    if (vendorName.includes('shop') || vendorName.includes('commerce')) return 'ecommerce';
    if (vendorName.includes('cf') || vendorName.includes('cloudflare') || vendorName.includes('cdn')) return 'cdn';
    if (categories.includes('security')) return 'security';
    if (categories.includes('caching')) return 'hosting';
    return 'framework';
  }
      ...
      if (headerData.frequency > 0.01) {
        ...
        if (expectedCategory && expectedCategory !== semanticAnalysis.category.primary) {
          anomalies.push({
            ...
            reason: `Header suggests ${expectedCategory} but categorized as ${semanticAnalysis.category.primary}`,
            severity: 'medium',
  ...
  private predictExpectedCategory(header: string): HeaderPrimaryCategory | undefined {
  ...
  private calculateDiscoveryMetrics(
    ...
    const avgConfidence = discoveredPatterns.size > 0 
      ? Array.from(discoveredPatterns.values()).reduce((sum, p) => sum + p.confidence, 0) / discoveredPatterns.size
      : 0;
  ...
  private trackValidationIntegration(
    ...
    const validatedPatternsUsed = Array.from(discoveredPatterns.values())
      .filter(p => p.validationConfidence && p.validationConfidence > 0.7).length;
