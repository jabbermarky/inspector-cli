...
import { generateRecommendationsV1 } from './recommender-v1.js';
import { DataPreprocessor } from './data-preprocessor-v2.js';
import { formatOutputV1 } from './reporter-v1.js';
import { analyzeDatasetBiasV1 } from './bias-detector-v1.js';
import { batchAnalyzeHeaders, generateSemanticInsights } from './semantic-analyzer-v1.js';
import { analyzeVendorPresence, inferTechnologyStack } from './vendor-patterns-v1.js';
import { analyzeHeaderCooccurrence } from './co-occurrence-analyzer-v1.js';
import { discoverHeaderPatterns } from './pattern-discovery-v1.js';
import { runStandardPipeline } from './analysis-pipeline-v1.js';
import type {
    FrequencyOptions,
    FrequencyResult,
    DetectionDataPoint,
 ...
 */
export async function analyzeFrequencyV1(options: FrequencyOptions = {}): Promise<FrequencyResult> {
    ...
    try {
        ...
        if (dataPoints.length < opts.minSites) {
            throw new Error(
                `Insufficient data: found ${dataPoints.length} sites, minimum required: ${opts.minSites}`
            );
        }
        ...
        const { analyzeScripts } = await import('./script-analyzer-v1.js');
        ...
        const biasAnalysis = await analyzeDatasetBiasV1(dataPoints, opts);
        ...
        if (opts.enableValidation && biasAnalysis.headerCorrelations.size > 0) {
            ...
            try {
                const pipelineResult = await runStandardPipeline(dataPoints, biasAnalysis, opts, {
                ...
                const significantCount = pipelineResult.finalData.significanceResults
                    ? Array.from(pipelineResult.finalData.significanceResults.values()).filter(
                          result => result.result?.isSignificant
                      ).length
                    : 0;
                ...
                if (
                    pipelineResult.overallPassed &&
                    pipelineResult.finalData.correlations.size > 0
                ) {
        ...
        if (opts.includeRecommendations) {
            ...
            recommendations = await generateRecommendationsV1({
                headerPatterns,
                metaPatterns,
                scriptPatterns,
                dataPoints,
        ...
        if (opts.outputFile) {
            await formatOutputV1(result, opts);
        }
...
function formatMetaTagData(
    ...
    for (const [metaKey, patterns] of metaPatterns.entries()) {
        ...
        const uniqueSitesUsingMeta = new Set(patterns.flatMap(p => p.examples)).size;
...
function formatScriptData(
    ...
    for (const [_category, patterns] of scriptPatterns.entries()) {
        for (const pattern of patterns) {
