...
import { runSanityChecks, type SanityChecksReport, type ValidationWarning } from './sanity-checks.js';
import { testMultipleCorrelations, type SignificanceTestResult } from './statistical-tests.js';
import type { 
...
export class AnalysisPipeline {
  ...
  async process(
    ...
    for (const stage of this.options.stages) {
      ...
      if (stage === AnalysisStage.FREQUENCY_FILTER || 
    ...
    const result: PipelineResult = {
      ...
      summary: {
        ...
        filterEfficiency: initialHeaderCount > 0 ? (initialHeaderCount - finalHeaderCount) / initialHeaderCount : 0,
  ...
  private async runStage(
    ...
    biasAnalysis: DatasetBiasAnalysis,
    originalData: DetectionDataPoint[]
  ): Promise<StageResult> {
    ...
    try {
      switch (stage) {
        ...
        case AnalysisStage.SANITY_CHECKS:
          return this.runSanityChecksStage(currentData, biasAnalysis, stageStartTime, initialCount);
          
        case AnalysisStage.SIGNIFICANCE_TEST:
          return this.runSignificanceTestStage(currentData, biasAnalysis, stageStartTime, initialCount);
          
        case AnalysisStage.RECOMMENDATION_GEN:
          return this.runRecommendationValidation(currentData, stageStartTime, initialCount);
      ...
      if (correlation.overallFrequency < this.options.frequencyThreshold) {
        ...
        if (this.options.debugMode) {
          warnings.push({
            ...
            headerName,
            details: { frequency: correlation.overallFrequency, threshold: this.options.frequencyThreshold }
          });
        }
      }
    }
      ...
      if (correlation.overallOccurrences < this.options.sampleSizeThreshold) {
        ...
        if (this.options.debugMode) {
          warnings.push({
            ...
            headerName,
            details: { sampleSize: correlation.overallOccurrences, threshold: this.options.sampleSizeThreshold }
          });
        }
      }
    }
      ...
      switch (correlation.recommendationConfidence) {
        ...
        case 'low':
          ...
          warnings.push({
            ...
            headerName,
            details: { confidence: correlation.recommendationConfidence, platformSpecificity: correlation.platformSpecificity }
          });
          break;
      }
    }
