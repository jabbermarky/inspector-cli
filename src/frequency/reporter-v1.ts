...
import type { DatasetBiasAnalysis } from './bias-detector-v1.js';
 ...
 */
export async function formatOutputV1(
    result: FrequencyResult,
    options: FrequencyOptionsWithDefaults,
    biasAnalysis?: DatasetBiasAnalysis | null
): Promise<void> {
    ...
    switch (options.output) {
        case 'json':
            ...
            break;
        case 'csv':
            content = formatAsCSVV1(result);
            break;
        case 'markdown':
            content = formatAsMarkdownV1(result, options, biasAnalysis);
            break;
        case 'human':
        default:
            content = formatAsHumanV1(result, options, biasAnalysis);
            break;
    }
 ...
 */
function formatAsHumanV1(
    result: FrequencyResult,
    options: FrequencyOptionsWithDefaults,
    biasAnalysisParam?: DatasetBiasAnalysis | null
): string {
    const {
        metadata,
        headers,
...
`;
        ...
        for (const [headerName, correlation] of biasAnalysis.headerCorrelations.entries()) {
            if (correlation.overallOccurrences >= 5) {
                // Only show headers with reasonable sample size
                output += `
#### \`${headerName}\`
...
`;
        for (const rec of recommendations.learn.recommendToFilter) {
            ...
            const freqPercent = headerData
                ? Math.round(headerData.frequency * 100)
                : Math.round(rec.frequency * 100);
...
`;
        ...
        const highCorrelationPairs = cooccurrenceAnalysis.cooccurrences
            ...
            .sort(
                (a: HeaderCooccurrence, b: HeaderCooccurrence) =>
        ...
        const exclusivePairs = cooccurrenceAnalysis.cooccurrences
            .filter(
                (h: HeaderCooccurrence) =>
                    h.cooccurrenceCount === 0 && h.cooccurrenceFrequency === 0
            )
            .sort(
                (a: HeaderCooccurrence, b: HeaderCooccurrence) =>
                    b.conditionalProbability +
                    a.conditionalProbability -
                    (a.conditionalProbability + b.conditionalProbability)
            )
            .slice(0, 10);
...
`;
        ...
        const conventions = Object.entries(semanticAnalysis.insights.namingConventions).sort(
            ([, a], [, b]) => b - a
        );

        // Calculate total from all convention counts
        const totalConventionHeaders = Object.values(
            semanticAnalysis.insights.namingConventions
        ...
        const patternTypes = Object.entries(semanticAnalysis.insights.patternTypes).sort(
            ([, a], [, b]) => b - a
        );

        // Calculate total from all pattern type counts
        const totalPatternHeaders = Object.values(semanticAnalysis.insights.patternTypes).reduce(
            (sum, count) => sum + count,
            0
        );
        ...
        if (semanticAnalysis.technologyStack) {
            ...
            if (
                semanticAnalysis.technologyStack.cdn &&
                semanticAnalysis.technologyStack.cdn.length > 0
            ) {
            ...
            if (
                semanticAnalysis.technologyStack.analytics &&
                semanticAnalysis.technologyStack.analytics.length > 0
            ) {
            ...
            if (
                semanticAnalysis.technologyStack.security &&
                semanticAnalysis.technologyStack.security.length > 0
            ) {
...
`;
        ...
        const prefixPatterns = patternDiscoveryAnalysis.discoveredPatterns.filter(
            p => p.type === 'prefix'
        ).length;
        const suffixPatterns = patternDiscoveryAnalysis.discoveredPatterns.filter(
            p => p.type === 'suffix'
        ).length;
        const containsPatterns = patternDiscoveryAnalysis.discoveredPatterns.filter(
            p => p.type === 'contains'
        ).length;
        const regexPatterns = patternDiscoveryAnalysis.discoveredPatterns.filter(
            p => p.type === 'regex'
        ).length;
...
`;
        ...
        const sortedCMS = Object.entries(biasAnalysis.cmsDistribution).sort(
...
`;
    ...
    const sortedHeaders = Object.entries(headers).sort(([, a], [, b]) => b.frequency - a.frequency);
...
`;
        for (const rec of recommendations.learn.recommendToKeep) {
            ...
            if (
                biasAnalysis &&
                biasAnalysis.headerCorrelations &&
                biasAnalysis.headerCorrelations.has(rec.pattern)
            ) {
                const correlation = biasAnalysis.headerCorrelations.get(rec.pattern)!;
                ...
                if (headerData) {
                    ...
                    if (classification.category === 'platform') {
                        ...
                        const platformName =
                            classification.vendor || classification.platformName || 'Platform';
...
`;
    ...
    if (cooccurrenceAnalysis) {
        ...
        if (cooccurrenceAnalysis.technologySignatures.length > 0) {
            ...
            for (const signature of cooccurrenceAnalysis.technologySignatures.slice(0, 10)) {
                const headers =
                    signature.requiredHeaders.length > 3
                        ? signature.requiredHeaders.slice(0, 3).join(', ') +
                          `... (${signature.requiredHeaders.length} total)`
        ...
        if (cooccurrenceAnalysis.platformCombinations.length > 0) {
            ...
            for (const combo of topCombinations) {
                const headers =
                    combo.headerGroup.length > 2
                        ? combo.headerGroup.slice(0, 2).join(' + ') +
                          `... (${combo.headerGroup.length})`
    ...
    if (semanticAnalysis) {
        ...
        if (semanticAnalysis.technologyStack) {
            ...
            if (
                ...
                const cdnDisplay =
                    semanticAnalysis.technologyStack.cdn.length > 8
            ...
            if (
                ...
                const analyticsDisplay =
                    semanticAnalysis.technologyStack.analytics.length > 6
    ...
    if (patternDiscoveryAnalysis) {
            ...
            for (const pattern of topPatterns) {
                ...
                const examplesDisplay =
                    (pattern.examples || []).length > 2
                        ? `${examples}... (+${(pattern.examples || []).length - 2})`
                        : examples;
        ...
        if (patternDiscoveryAnalysis.emergingVendors.length > 0) {
            ...
            for (const vendor of patternDiscoveryAnalysis.emergingVendors.slice(0, 10)) {
                ...
                const prefixDisplay =
                    vendor.characteristics.commonPrefixes.length > 3
        ...
        if (patternDiscoveryAnalysis.patternEvolution.length > 0) {
            ...
            for (const evolution of patternDiscoveryAnalysis.patternEvolution.slice(0, 12)) {
                const latestVersion = evolution.versions[evolution.versions.length - 1];
                const latestFreq = latestVersion
                    ? Math.round(latestVersion.frequency * 100) + '%'
                    : 'N/A';
...
`;
        ...
        for (const [pattern, data] of patterns.slice(0, 15)) {
            ...
            const frequencyPercent = Math.round(data.frequency * 100);
            const firstExample =
                data.examples && data.examples[0] ? formatScriptExample(data.examples[0]) : 'N/A';
 ...
 */
function formatAsCSVV1(result: FrequencyResult): string {
    ...
    for (const [headerName, data] of Object.entries(result.headers)) {
        for (const value of data.values || []) {
            ...
            const pattern = headerName.endsWith(`:${value.value}`)
                ? headerName
