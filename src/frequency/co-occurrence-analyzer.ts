...
import { findVendorByHeader, type VendorPattern } from './vendor-patterns.js';
import { analyzeHeaderSemantics, type HeaderSemanticAnalysis } from './semantic-analyzer.js';
...
export function analyzeHeaderCooccurrence(dataPoints: DetectionDataPoint[]): CooccurrenceAnalysis {
  ...
  const technologySignatures = identifyTechnologyStackSignatures(meaningfulCooccurrences, dataPoints);
  ...
  const strongCorrelations = meaningfulCooccurrences.filter(c => 
    c.mutualInformation > 0.1 && c.conditionalProbability > 0.7
...
function calculateCooccurrencePairs(
  ...
  for (let i = 0; i < headers.length; i++) {
    for (let j = i + 1; j < headers.length; j++) {
      ...
      const mutualInformation = cooccurrenceFrequency > 0 
 ...
 */
function findPlatformSpecificCombinations(dataPoints: DetectionDataPoint[]): PlatformHeaderCombination[] {
...
function detectMutuallyExclusiveGroups(cooccurrences: HeaderCooccurrence[]): string[][] {
  ...
  const lowCooccurrencePairs = cooccurrences.filter(c => 
  ...
  for (const [header, exclusives] of exclusivityMap.entries()) {
    ...
    if (group.length >= 3) { // Only include groups with 3+ headers
...
export function generateCooccurrenceInsights(analysis: CooccurrenceAnalysis): string[] {
  ...
  if (analysis.mutuallyExclusiveGroups.length > 0) {
    const largestGroup = analysis.mutuallyExclusiveGroups.reduce((a, b) => a.length > b.length ? a : b);
    insights.push(
