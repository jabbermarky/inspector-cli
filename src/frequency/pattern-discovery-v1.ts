 ...
 */
function buildHeaderFrequencyMap(dataPoints: DetectionDataPoint[]): Map<
    string,
    {
        frequency: number;
        sites: string[];
...
function extractNamingPatterns(
    headerFrequency: Map<
        string,
        { frequency: number; sites: string[]; cmsCorrelation: Record<string, number> }
    >,
    ...
    const minFrequency = dataPoints.length >= 100 ? 0.05 : 0.1; // Lower threshold for small datasets
    return patterns
...
function discoverPrefixPatterns(
    ...
    for (const header of headers) {
        for (let len = 2; len <= Math.min(12, header.length - 2); len++) {
            // -2 to ensure meaningful suffix after prefix
    ...
    for (const [prefix, matchingHeaders] of prefixMap.entries()) {
        if (matchingHeaders.length >= 2) {
            // At least 2 headers with this prefix
...
function discoverContainsPatterns(
    ...
    for (const [token, matchingHeaders] of tokenMap.entries()) {
        if (matchingHeaders.length >= 3) {
            // At least 3 headers with this token
            const allSites = new Set<string>();
            const cmsCorrelation: Record<string, number> = {};
...
function identifyEmergingVendors(
    ...
    for (const [vendorName, candidate] of vendorCandidates.entries()) {
        if (candidate.patterns.length >= 2) {
            // At least 2 patterns for a vendor
            ...
            const confidence =
                candidate.patterns.reduce((sum, p) => sum + p.confidence, 0) /
...
function detectSemanticAnomalies(
    ...
    for (const [header, data] of headerFrequency.entries()) {
        if (data.frequency > 0.01) {
            // Only analyze reasonably common headers
...
function inferVendorFromPattern(pattern: DiscoveredPattern): string | undefined {
    ...
    for (const vendor of vendorIndicators) {
        if (
            vendor.patterns.some(p => patternName.includes(p) || examples.some(e => e.includes(p)))
        ) {
            return vendor.name;
        }
    }
...
function predictExpectedCategory(header: string, knownVendor: any): string | undefined {
    ...
    if (
        lowerHeader.includes('security') ||
        lowerHeader.includes('csp') ||
        lowerHeader.includes('hsts')
    )
        return 'security';
