...
export class StatisticalTester {
  ...
  static testSignificance(
    ...
    const contingencyData = this.createContingencyTable(correlation, cmsDistribution, totalSites);
    ...
    if (totalObservations <= this.FISHER_MAX_TOTAL || 
  ...
  private static createContingencyTable(
    ...
    if (a < 0 || b < 0 || c < 0 || d < 0) {
      logger.warn('Invalid contingency table', { headerName: correlation.headerName, bestCms, a, b, c, d });
      return null;
    }
