...
function extractExternalScriptPatterns(scriptSrc: string): string[] {
    ...
    try {
        ...
        if (hostname.includes('google-analytics') || pathname.includes('gtag'))
            patterns.push('tracking:google-analytics');
        if (hostname.includes('googletagmanager') || pathname.includes('gtm'))
            patterns.push('tracking:google-tag-manager');
        if (hostname.includes('facebook') || filename.includes('fbevents'))
            patterns.push('tracking:facebook');
...
function extractInlineScriptPatterns(content: string): string[] {
    ...
    if (content.includes('wp-admin') || content.includes('WordPress'))
        patterns.push('inline:wordpress');
    ...
    if (content.includes('gtag(') || content.includes('ga('))
        patterns.push('inline:google-analytics');
    ...
    if (content.includes('window.Parameters') || content.includes('DUDAONE'))
        patterns.push('inline:duda');
    ...
    if (content.includes('document.addEventListener') || content.includes('DOMContentLoaded'))
        patterns.push('inline:dom-ready');
