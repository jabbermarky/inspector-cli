...
function groupSitesByCMS(dataPoints: DetectionDataPoint[]): Map<string, DetectionDataPoint[]> {
  ...
  for (const site of dataPoints) {
    ...
    if (site.detectionResults && site.detectionResults.length > 0) {
      ...
      if (bestResult.confidence >= 0.5) { // Only trust results with 50%+ confidence
        cms = bestResult.cms;
      }
    }
...
async function analyzePatternsByCMS(
  ...
  const globalPatternCounts = new Map<string, {
    totalSites: number;
  ...
  for (const [pattern, data] of globalPatternCounts.entries()) {
    ...
    const { confidenceLevel, recommendedConfidence } = determineConfidenceLevel(
      pattern, cmsCorrelation, discriminativePower, frequency
    );
 ...
 */
function extractEnhancedScriptPatterns(scripts: any[], siteUrl: string): Array<{
  pattern: string;
  example: string;
  sameDomain: boolean;
}> {
...
function shouldBeSameDomainOnly(pattern: string): boolean {
  ...
  const sameDomainPatterns = [
    'path:wp-content', 'path:wp-includes', 'path:wp-admin',
    'path:sites', 'path:core', 'path:modules',
    'path:administrator', 'path:components', 'path:templates', 'path:media',
    'path:assets', 'path:js'
  ];
    ...
    for (const pattern of categoryPatterns) {
      ...
      if (pattern.discriminativePower < 0.3 && pattern.frequency > 0.3) {
        patternRefinements.push({
          ...
          affectedCMS: Object.keys(pattern.cmsCorrelation).filter(cms => pattern.cmsCorrelation[cms] > 0.1)
        });
      }
    }
  }
