...
export async function generateRecommendationsV1(input: RecommendationInput): Promise<{
    ...
    if (!biasAnalysis) {
        ...
        biasAnalysis = await analyzeDatasetBiasV1(input.dataPoints, input.options);
    }

    // Generate bias-aware recommendations
    const inputWithBias = { ...input, biasAnalysis };
    const learnRecommendations = generateLearnRecommendationsV1(inputWithBias);
    const detectCmsRecommendations = generateDetectCmsRecommendationsV1(inputWithBias);
    const groundTruthRecommendations = generateGroundTruthRecommendationsV1(inputWithBias);
 ...
 */
function generateLearnRecommendationsV1(
    input: RecommendationInput & { biasAnalysis: DatasetBiasAnalysis }
): LearnRecommendations {
    ...
    for (const [headerName, patterns] of headerPatterns.entries()) {
        ...
        if (!isCurrentlyFiltered) {
            ...
            if (shouldFilter) {
                ...
                const shouldKeep =
                    classification.filterRecommendation === 'never-filter' ||
                    (useBiasAware &&
                        correlation &&
                        shouldKeepBasedOnCorrelation(
                            headerName,
                            correlation,
                            totalFrequency,
                            diversity,
 ...
 */
function generateDetectCmsRecommendationsV1(
    input: RecommendationInput & { biasAnalysis: DatasetBiasAnalysis }
): DetectCmsRecommendations {
    ...
    for (const [headerName, patterns] of headerPatterns.entries()) {
        ...
        if (topCMS && topCMS[1].probability > 0.5 && topCMS[1].count >= 10) {
            ...
            if (totalFrequency >= 0.01 && totalFrequency <= 0.3) {
                // 1-30% frequency range
    ...
    newPatternOpportunities.sort(
        (a, b) => b.confidence * (1 - b.frequency) - a.confidence * (1 - a.frequency)
    );
 ...
 */
function generateGroundTruthRecommendationsV1(
        ...
        for (const pattern of patterns) {
            ...
            if (pattern.frequency >= 0.02 && pattern.frequency <= 0.3) {
                ...
                const examplesArray = Array.isArray(pattern.examples)
                    ? pattern.examples
                    : Array.from(pattern.examples || []);
                const hasDistinctiveValues = examplesArray.some(example =>
                    /wordpress|drupal|joomla|shopify|woocommerce|magento|prestashop/i.test(
                        String(example)
                    )
                );
...
function getFilterReason(frequency: number, diversity: number, maxFrequency: number): string {
    ...
    if (diversity > 20)
        return `High diversity (${diversity} unique values) suggests non-discriminative`;
...
function getKeepReason(frequency: number, diversity: number, maxFrequency: number): string {
    if (maxFrequency < 0.2)
        return `Low frequency (${Math.round(maxFrequency * 100)}%) - insufficient data for correlation analysis`;
    if (diversity <= 3)
        return `Low diversity (${diversity} values) may indicate platform-specific usage`;
...
function getFilterReasonBiasAware(
    ...
    const classification = preprocessor.classifyHeader(headerName);
    if (
        (classification.category === 'infrastructure' || classification.category === 'generic') &&
        correlation.platformSpecificity < 0.2
    ) {
...
function getKeepReasonBiasAware(
    ...
    if (
        topCMSByProbability &&
        topCMSByProbability[1].probability > 0.3 &&
