...
function isBotDetectionPage(dataPoint: DetectionDataPoint): boolean {
    ...
    const title =
        dataPoint.metaTags?.find(tag => tag.name === 'title')?.content?.toLowerCase() || '';
...
function isErrorPage(dataPoint: DetectionDataPoint): boolean {
    ...
    const hasSimpleErrorIndicator =
        cleanedLower.includes('404') || cleanedLower.includes('not found');
...
function hasRobotsTxtData(dataPoint: DetectionDataPoint): boolean {
    return !!(
        dataPoint.robotsTxt?.httpHeaders && Object.keys(dataPoint.robotsTxt.httpHeaders).length > 0
    );
}
