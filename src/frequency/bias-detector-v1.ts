 ...
 */
export async function analyzeDatasetBiasV1(
    ...
    const headerCorrelations = analyzeHeaderCMSCorrelations(
        dataPoints,
        cmsDistribution,
        options,
        preprocessor
...
function calculateCMSDistribution(dataPoints: DetectionDataPoint[]): CMSDistribution {
    ...
    for (const dataPoint of dataPoints) {
        ...
        if (detectedCms === 'Unknown') {
            const category = calculateWebsiteCategoryV1(
                dataPoint.httpHeaders,
...
function generateBiasWarnings(
    ...
    if (concentrationScore > 0.6) {
        warnings.push(
            `High dataset concentration (${Math.round(concentrationScore * 100)}%) - recommendations may be biased`
        );
    }
    ...
    for (const [cms, data] of Object.entries(cmsDistribution)) {
        if (data.percentage > 60) {
            warnings.push(
                `Dataset heavily skewed toward ${cms} (${Math.round(data.percentage)}%) - high-frequency headers may be platform-specific`
            );
        }
    }
    ...
    if (cmsTypes <= 2) {
        warnings.push(
            `Low CMS diversity (${cmsTypes} types) - may not represent general web patterns`
        );
    }
    ...
    if (unknownPercentage > 30) {
        warnings.push(
            `High percentage of unidentified sites (${Math.round(unknownPercentage)}%) - detection accuracy may be low`
        );
    }
...
function analyzeHeaderCMSCorrelations(
    ...
    for (const [headerName, cmsStats] of headerStats.entries()) {
        const overallOccurrences = Array.from(cmsStats.values()).reduce(
            (sum, urlSet) => sum + urlSet.size,
            0
        );
        const overallFrequency = overallOccurrences / totalSites;

        const perCMSFrequency: Record<
            string,
            { frequency: number; occurrences: number; totalSitesForCMS: number }
        > = {};
        ...
        const platformSpecificity = calculatePlatformSpecificity(
            perCMSFrequency,
            cmsDistribution,
            cmsGivenHeader,
            overallOccurrences
        ...
        const biasAdjustedFrequency = calculateBiasAdjustedFrequency(
            perCMSFrequency,
            cmsDistribution
        );
...
function calculatePlatformSpecificity(
    ...
    if (overallOccurrences >= 30) {
        ...
        const overallFrequency =
            overallOccurrences /
            Object.values(cmsDistribution).reduce((sum, d) => sum + d.count, 0);
        const backgroundContrast =
            topCMSFrequency > 0
                ? Math.min(2, topCMSFrequency / Math.max(overallFrequency, 0.001))
                : 0;
...
function calculateBiasAdjustedFrequency(
    ...
    const majorCMSTypes = Object.keys(cmsDistribution).filter(
        cms => cms !== 'Unknown' && cmsDistribution[cms].percentage > 5
    );
...
function assessRecommendationConfidence(
    ...
    if (
        dominantCMS &&
        dominantCMS[1].frequency > 0.8 &&
        cmsDistribution[dominantCMS[0]]?.percentage > 50
    ) {
        recommendationConfidence = 'low';
 ...
 */
// function isEnterpriseInfrastructureHeaderV1(headerName: string): boolean {
...
//     return classification.category === 'infrastructure' || classification.category === 'generic';
// }
...
function calculateWebsiteCategoryV1(
    ...
    if (detectionResults && detectionResults.length > 0) {
        const bestDetection = detectionResults.sort((a, b) => b.confidence - a.confidence)[0];
