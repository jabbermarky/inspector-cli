...
/**
 * Call the OpenAI assistant API with screenshot files
 */
export async function processAssistantRequest(screenshots: string[], options: AssistantOptions = {}): Promise<void> {
        try {
        ...
        logger.info('Starting assistant API call', { screenshots: screenshots.length, options });
        const params = {
            model: options.model,
            screenshot: screenshots,
            assistant: options.assistant,
            temperature: options.temperature,
            top_p: typeof options.top_p === 'string' ? parseFloat(options.top_p) : options.top_p,
        };
                ...
                if (message.role === "assistant") {
                    ...
                    if (content.type === "text") {
                        ...
                        if (validJSON(valueString) && validateBrandiJsonSchema(valueString)) {
                            ...
                            if (options.outfile) {
                                fs.writeFileSync(options.outfile, JSON.stringify(output, null, 2));
                                console.log(`Output written to ${options.outfile}`);
                                logger.info('Output file written', { outfile: options.outfile });
                            }
                        }
                        break;
                    }
