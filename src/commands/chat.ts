...
/**
 * Call the OpenAI chat API with screenshot files
 */
export async function processChatRequest(screenshots: string[], options: ChatOptions = {}): Promise<void> {
    try {
...
program
    ...
    .action(async (screenshots: string[], _options) => {
        await processChatRequest(screenshots, { model: _options.model });
    });
